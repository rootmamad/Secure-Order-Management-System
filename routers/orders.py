...
def add_to_cart(
    ...
    if not item:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="این محصول موجود نیست"
        )

    if quantity <= 0:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="موجودی نمیتواند منفی باشد."
        )

    if item.quantity < quantity:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"موجودی کالا کافی نیست. موجودی فعلی: {item.quantity}",
        )

    if quantity * item.price > user.balance:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="اعتبار حساب کافی نیست."
        )

    order = (
        session.query(Order)
        .filter_by(user_id=user.id, status="pending", kind="buy")
        .first()
    )
    if not order:
    ...
    session.flush()
    return {
        "message": f"تعداد {quantity} عدد از {item.name} به لیست خریدهای شما اضافه شد"
    }
...
def myitem(
    ...
    calculator = func.sum(
        case(
            (Order.kind == "buy", OrderItem.quantity),
            (Order.kind == "return", -OrderItem.quantity),
            else_=0,
        )
    )
    results = (
        session.query(OrderItem.item_id, calculator.label("quantity"), Items)
        .join(Order, Order.id == OrderItem.order_id)
        .join(Items, Items.id == OrderItem.item_id)
        .filter(Order.status == "completed", Order.user_id == current_user["user_id"])
        .group_by(OrderItem.item_id, Items.id)
        .having(calculator > 0)
        .all()
    )
    if not results:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=" محصولی موجود نیست"
        )
    items = []
    for row in results:
        items.append(
            {"item_id": row.item_id, "quantity": row.quantity, "item": row.Items}
        )
    return items
...
def return_item(
    ...
    if item not in myitems_:
        raise HTTPException(
            ...
            detail="این محصول در لیست خرید شما وجود ندارد",
        )
    ...
    if quantity > quantities[myitems_.index(item)]:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"شما فقط {quantities[myitems_.index(item)]} عدد از این محصول را خریداری کرده اید.",
        )
    ...
    order_item = OrderItem(
        order_id=order.id, item_id=item.id, quantity=quantity, price_at_buy=item.price
    )
...
def get_order(
    ...
    if not order:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="سفارش موجود نیست."
        )
...
def checkout(
    ...
    user = (
        session.query(Users).with_for_update().filter(Users.id == order.user_id).first()
    )
    if order.total_amount > user.balance:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Insufficient balance to complete the checkout.",
        )
    ...
    for item_ in order_items:
        ...
        if not item:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Item with ID {item_.item_id} no longer exists.",
            )
        if item_.quantity > item.quantity:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Insufficient stock for '{item.name}'. Only {item.quantity} remaining (requested: {item_.quantity}).",
            )
...
def cancel(
    ...
    if order.status != "pending":
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="This order has already been completed or canceled.",
        )
