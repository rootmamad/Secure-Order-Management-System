...
def get_all_users(request:Request,offset:int=0,limit:int=10,session: Session = Depends(get_session),current_user: dict = Depends(require_admin_role)):
    users = session.query(Users).limit(limit).offset(offset).all()
    return users

@router.patch("/users/role/{user_id}")
def update_role(request_:Request,user_id:int,request:RoleUpdateRequest,session:Session =Depends(get_session),current_user:dict=Depends(require_admin_role)):    
