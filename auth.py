...
from utils import create_hash, create_token, verify_hash
...
def register(
    ...
    refresh_token = create_token(
        {"user_id": db_user.id, "username": db_user.username, "is_refresh": True},
        settings.secret_key,
        settings.algorithm,
    )
...
@limiter.limit("15/minute")
def login(
    request: Request, user: UserLogin, session: Session = Depends(get_session)
) -> LoginResponse:
    ...
    last_refresh_token = (
        session.query(RefreshTokens)
        .filter(RefreshTokens.user_id == db_user.id)
...
def refresh_token(
    request: Request,
    request_: RefreshTokenRequest,
    session: Session = Depends(get_session),
) -> Token:
    try:
        payload = jwt.decode(
            request_.refresh_token, settings.secret_key, algorithms=[settings.algorithm]
        )
        if not payload.get("is_refresh"):
            raise HTTPException(
                status_code=401, detail="این اکسس توکنه، رفرش توکن بده!"
            )
    ...
    old_refresh_token = (
        session.query(RefreshTokens)
        .filter(RefreshTokens.token == request_.refresh_token)
        .first()
    )
